...
void sms_state::machine_reset()
{
	...
	if (!m_is_mark_iii && !m_is_gamegear && retrodepth_active() && m_cartslot && m_cartslot->exists())
	{
		// RetroDepth wants the cart to boot directly so strict BIOS cart checks
		// do not block otherwise valid dumps with a SOFTWARE ERROR screen.
		m_mem_ctrl_reg &= ~IO_CARTRIDGE;
		m_mem_ctrl_reg |= IO_BIOS_ROM;
		m_maincpu->set_state_int(Z80_SP, 0xdff0);
	}
