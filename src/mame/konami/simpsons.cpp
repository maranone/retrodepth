...
uint32_t simpsons_state::screen_update(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	if (retrodepth_active())
	{
		...
		{
			palette_device* pal_dev = subdevice<palette_device>("palette");
			const rgb_t* colors = pal_dev->palette()->entry_list_adjusted();
			const int n_pens = (int)pal_dev->palette()->num_colors();
