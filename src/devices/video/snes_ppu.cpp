...
// Per-layer per-scanline pixel capture for RetroDepth layer separation.
// Indexed [bucket][x]:
//   0 background (backdrop + BG4), 1 BG3,
//   2/3 BG2 low/high, 4/5 BG1 low/high, 6/7 OAM low/high.
// 0xFFFF (bit 15 set) = no pixel from this layer at this column.
enum
{
	RD_SNES_BG = 0,
	RD_SNES_BG3,
...
static uint16_t s_rd_line_main[RD_SNES_BUCKETS][SNES_SCR_WIDTH];
static uint16_t s_rd_line_sub[RD_SNES_BUCKETS][SNES_SCR_WIDTH];

static inline void snes_rd_capture_pixel(uint16_t (&target)[RD_SNES_BUCKETS][SNES_SCR_WIDTH], int bucket, uint16_t x, uint16_t color)
{
	if (bucket < 0)
		return;
	target[bucket][x] = color;
}
...
inline void snes_ppu_device::plot_above( uint16_t x, uint8_t source, uint8_t priority, uint16_t color, int blend_exception )
{
	...
	// RetroDepth: capture each layer's pixels before compositing.
	if (retrodepth_active())
	{
		int bucket = -1;
		switch (source)
		{
		case SNES_BG1: bucket = (priority >= m_layer[SNES_BG1].priority[1]) ? RD_SNES_BG1_HIGH : RD_SNES_BG1_LOW; break;
		case SNES_BG2: bucket = (priority >= m_layer[SNES_BG2].priority[1]) ? RD_SNES_BG2_HIGH : RD_SNES_BG2_LOW; break;
		case SNES_BG3: bucket = RD_SNES_BG3; break;
		case SNES_BG4: bucket = RD_SNES_BG; break;
		case SNES_OAM: bucket = (priority >= m_oam.priority[2]) ? RD_SNES_OBJ_HIGH : RD_SNES_OBJ_LOW; break;
		default: break;
		}
		snes_rd_capture_pixel(s_rd_line_main, bucket, x, color);
	}
}
...
inline void snes_ppu_device::plot_below( uint16_t x, uint8_t source, uint8_t priority, uint16_t color, int blend_exception )
{
	...
	// Some SNES effects only exist on the sub-screen for color math. If the
	// main-screen capture alone is exported, those effects disappear in VR.
	// Keep a separate sub-screen capture so export can reconstruct them.
	if (retrodepth_active())
	{
		int bucket = -1;
		switch (source)
		...
		snes_rd_capture_pixel(s_rd_line_sub, bucket, x, color);
	}
}
...
void snes_ppu_device::refresh_scanline( bitmap_rgb32 &bitmap, uint16_t curline )
{
	...
	else
	{
		...
		if (retrodepth_active())
		{
			memset(s_rd_line_main, 0xff, sizeof(s_rd_line_main)); // 0xFFFF = no layer pixel yet
			memset(s_rd_line_sub, 0xff, sizeof(s_rd_line_sub));
		}
		...
		// Accumulates per-layer pixel data scanline by scanline.
		// Layers: background (backdrop + BG4), BG3, BG2 low/high,
		// BG1 low/high, sprites low/high.
		if (retrodepth_active())
		{
			static bitmap_rgb32 s_layers[RD_SNES_BUCKETS];

			if (curline == 1)
			{
				for (int i = 0; i < RD_SNES_BUCKETS; i++)
				{
					if (s_layers[i].width() != SNES_SCR_WIDTH || s_layers[i].height() < 240)
						s_layers[i].allocate(SNES_SCR_WIDTH, 240);
					s_layers[i].fill(1u);
				}
			}

			int rd_row = (int)curline - 1;
			if (rd_row >= 0 && rd_row < 240)
			{
				// Backdrop color: pen_indirect(0) gives 15-bit BGR of CGRAM[0]
				uint16_t bd = pen_indirect(0);

				auto source_for_bucket = [&](int bucket) -> uint8_t
				{
					switch (bucket)
					{
					case RD_SNES_BG:       return SNES_BG4;
					case RD_SNES_BG3:      return SNES_BG3;
					case RD_SNES_BG2_LOW:
					case RD_SNES_BG2_HIGH: return SNES_BG2;
					case RD_SNES_BG1_LOW:
					case RD_SNES_BG1_HIGH: return SNES_BG1;
					case RD_SNES_OBJ_LOW:
					case RD_SNES_OBJ_HIGH: return SNES_OAM;
					default:               return SNES_COLOR;
					}
				};
				auto rgb32_from_snes = [](uint16_t c15) -> uint32_t
				{
					uint32_t rr = ((uint32_t)(c15 >>  0) & 0x1fu) << 3;
					uint32_t gg = ((uint32_t)(c15 >>  5) & 0x1fu) << 3;
					uint32_t bb2 = ((uint32_t)(c15 >> 10) & 0x1fu) << 3;
					return (rr << 16) | (gg << 8) | bb2;
				};
				auto resolve_bucket_color = [&](int bucket, uint16_t main_c15, uint16_t sub_c15, bool& has_pixel) -> uint32_t
				{
					const bool has_main = !(main_c15 & 0x8000u);
					const bool has_sub  = !(sub_c15  & 0x8000u);
					has_pixel = has_main || has_sub;
					if (!has_pixel)
						return 1u;

					uint16_t out_c15 = has_main ? main_c15 : sub_c15;
					const uint8_t source = source_for_bucket(bucket);
					if (has_main && source != SNES_OAM && source != SNES_COLOR && m_layer[source].color_math)
					{
						const bool halve = BIT(m_color_modes, 0) != 0;
						if (!m_sub_add_mode)
						{
							out_c15 = blend(main_c15, pen_indirect(FIXED_COLOUR), halve);
						}
						else if (has_sub)
						{
							out_c15 = blend(main_c15, sub_c15, halve);
						}
					}
					return rgb32_from_snes(out_c15);
				};
				auto resolve_backdrop_color = [&](int x) -> uint32_t
				{
					uint16_t out_c15 = bd;
					if (!window_above[x])
						out_c15 = 0;
					else if (window_below[x] && m_layer[SNES_COLOR].color_math)
					{
						if (!m_sub_add_mode)
						{
							out_c15 = blend(bd, pen_indirect(FIXED_COLOUR),
								BIT(m_color_modes, 0) != 0 && window_above[x] != 0);
						}
						else
						{
							out_c15 = blend(bd, below->buffer[x],
								BIT(m_color_modes, 0) != 0 &&
								window_above[x] != 0 &&
								below->layer[x] != SNES_COLOR);
						}
					}
				...
				for (int x = 0; x < SNES_SCR_WIDTH; x++)
				{
					// Background bucket defaults to the current backdrop colour, with BG4
					// pixels overriding it when that layer is active.
					s_layers[RD_SNES_BG].pix(rd_row, x) = resolve_backdrop_color(x);
					bool has_bg_pixel = false;
					uint32_t bg_pixel = resolve_bucket_color(
						RD_SNES_BG,
						s_rd_line_main[RD_SNES_BG][x],
						s_rd_line_sub[RD_SNES_BG][x],
						has_bg_pixel);
					if (has_bg_pixel)
					{
						s_layers[RD_SNES_BG].pix(rd_row, x) = bg_pixel;
					}

					// Layers 1..7: resolve main/sub captures into the closest visible colour
					// for this hardware bucket. Bit 15 set => no pixel from that layer.
					for (int li = 1; li < RD_SNES_BUCKETS; li++)
					{
						bool has_pixel = false;
						uint32_t pixel = resolve_bucket_color(
							li,
							s_rd_line_main[li][x],
							s_rd_line_sub[li][x],
							has_pixel);
						if (!has_pixel)
						{
							s_layers[li].pix(rd_row, x) = 1u; // transparent sentinel
						}
						else
						{
							s_layers[li].pix(rd_row, x) = pixel;
						}
					}
				}
			}

			// Export at end of visible area: curline==225 (NTSC) or curline==240 (PAL)
			if (curline == 225 || curline == 240)
			{
				const int w = SNES_SCR_WIDTH;
				const int h = (int)curline - 1; // 224 or 239 visible rows

				static std::vector<uint32_t> rd_packed;
				rd_packed.resize((size_t)w * h);

				static const char* k_names[RD_SNES_BUCKETS] = {
					"background",
					"bg3",
					"bg2_low",
					"bg2_high",
				...
				for (int li = 0; li < RD_SNES_BUCKETS; li++)
				{
					for (int r2 = 0; r2 < h; r2++)
						for (int c2 = 0; c2 < w; c2++)
							rd_packed[(size_t)r2 * w + c2] = s_layers[li].pix(r2, c2);
					retrodepth_write_layer((uint32_t)li, k_names[li], rd_packed.data(), nullptr, (uint32_t)w, (uint32_t)h);
				}
				retrodepth_commit();
			}
		}
