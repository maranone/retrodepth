...
// RetroDepth per-scanline layer capture for the Sega 315-5313 VDP.
//   0 = Plane B, 1 = Plane A + Window, -1 = not capturing
static int s_rd_gen_plane = -1;
static u8  s_rd_gen_capb_low[320];   // Plane B low-priority raw palette entries
static u8  s_rd_gen_capb_high[320];  // Plane B high-priority raw palette entries
static u8  s_rd_gen_capa_low[320];   // Plane A+Window low-priority raw palette entries
static u8  s_rd_gen_capa_high[320];  // Plane A+Window high-priority raw palette entries
static u8  s_rd_gen_spr_low[320];    // Sprite low-priority raw palette entries
static u8  s_rd_gen_spr_high[320];   // Sprite high-priority raw palette entries
enum
{
	RD_GEN_BG = 0,
	RD_GEN_PLANE_B_LOW,
...
static bitmap_rgb32 s_rd_gen_layers[RD_GEN_BUCKETS];
...
void sega315_5313_device::render_spriteline_to_spritebuffer(int scanline)
{
	...
	{
		memset(s_rd_gen_spr_low,  0, sizeof(s_rd_gen_spr_low));
		memset(s_rd_gen_spr_high, 0, sizeof(s_rd_gen_spr_high));
	}
					...
					for (int xtile = 0; xtile < width; xtile++)
					{
						...
						if (!xflip)
						{
							for (int loopcount = 0; loopcount < gfx_w; loopcount++)
							{
								...
								if (retrodepth_active() && dat)
								{
									const int vis_x = xxx - 128;
									if (vis_x >= 0 && vis_x < 320)
									{
										u8* dst = (pri & 0x80) ? s_rd_gen_spr_high : s_rd_gen_spr_low;
										if (!dst[vis_x]) dst[vis_x] = dat | (colour << 4);
									}
								}
...
inline void sega315_5313_device::draw_tile(nametable_t tile, int start, int end, int &dpos, bool is_fg)
{
	if (!tile.xflip)
	{
		...
		for (int shift = start; shift < end; shift++)
		{
			...
			if (retrodepth_active() && dat && dpos < 320)
			{
				const u8 cap = dat | (tile.colour << 4);
				if (s_rd_gen_plane == 0)
				{
					if (tile.pri) s_rd_gen_capb_high[dpos] = cap;
					else          s_rd_gen_capb_low[dpos]  = cap;
				}
				else if (s_rd_gen_plane == 1)
				{
					if (tile.pri) s_rd_gen_capa_high[dpos] = cap;
					else          s_rd_gen_capa_low[dpos]  = cap;
				}
			}
			dpos++;
		}
...
//  const u8  hscroll_mode = MEGADRIVE_REG0B_HSCROLL_MODE;
	...
	{
		s_rd_gen_plane = 0;
		memset(s_rd_gen_capb_low,  0, sizeof(s_rd_gen_capb_low));
		memset(s_rd_gen_capb_high, 0, sizeof(s_rd_gen_capb_high));
		memset(s_rd_gen_capa_low,  0, sizeof(s_rd_gen_capa_low));
		memset(s_rd_gen_capa_high, 0, sizeof(s_rd_gen_capa_high));
	}
	...
	if (retrodepth_active()) s_rd_gen_plane = 1;
...
void sega315_5313_device::render_videobuffer_to_screenbuffer(int scanline)
{
	...
	// --- RetroDepth Genesis layer export ---
	if (retrodepth_active())
	{
		const int sh = m_visible_scanlines;
		const int w  = (int)horz;

		// Initialise bitmaps and background layer on the first visible scanline.
		if (scanline == 0)
		{
			for (int li = 0; li < RD_GEN_BUCKETS; li++)
			{
				if (s_rd_gen_layers[li].width() != w || s_rd_gen_layers[li].height() != sh)
					s_rd_gen_layers[li].allocate(w, sh);
				s_rd_gen_layers[li].fill(1u); // transparent sentinel
			}
		}

		if (scanline >= 0 && scanline < sh)
		{
			const u8 bgentry = MEGADRIVE_REG07_BGCOLOUR;
			const u32 bg_pixel = (u32)m_gfx_palette->pen(bgentry + palette_per_scanline);
			for (int x = 0; x < w; x++)
			{
				s_rd_gen_layers[RD_GEN_BG].pix(scanline, x) = bg_pixel;

				u8 pb_low = s_rd_gen_capb_low[x];
				s_rd_gen_layers[RD_GEN_PLANE_B_LOW].pix(scanline, x) =
					pb_low ? (u32)m_gfx_palette->pen(pb_low + palette_per_scanline) : 1u;

				u8 pb_high = s_rd_gen_capb_high[x];
				s_rd_gen_layers[RD_GEN_PLANE_B_HIGH].pix(scanline, x) =
					pb_high ? (u32)m_gfx_palette->pen(pb_high + palette_per_scanline) : 1u;

				u8 pa_low = s_rd_gen_capa_low[x];
				s_rd_gen_layers[RD_GEN_PLANE_A_LOW].pix(scanline, x) =
					pa_low ? (u32)m_gfx_palette->pen(pa_low + palette_per_scanline) : 1u;

				u8 pa_high = s_rd_gen_capa_high[x];
				s_rd_gen_layers[RD_GEN_PLANE_A_HIGH].pix(scanline, x) =
					pa_high ? (u32)m_gfx_palette->pen(pa_high + palette_per_scanline) : 1u;

				u8 spr_low = s_rd_gen_spr_low[x];
				s_rd_gen_layers[RD_GEN_SPR_LOW].pix(scanline, x) =
					spr_low ? (u32)m_gfx_palette->pen(spr_low + palette_per_scanline) : 1u;

				u8 spr_high = s_rd_gen_spr_high[x];
				s_rd_gen_layers[RD_GEN_SPR_HIGH].pix(scanline, x) =
					spr_high ? (u32)m_gfx_palette->pen(spr_high + palette_per_scanline) : 1u;
			}
		}

		// Commit all layers at the end of the visible area.
		if (scanline == sh - 1)
		{
			static std::vector<u32> rd_packed;
			rd_packed.resize((size_t)w * sh);
			static const char* k_names[RD_GEN_BUCKETS] = {
				"background",
				"plane_b_low",
				"plane_b_high",
				"plane_a_low",
			...
			for (int li = 0; li < RD_GEN_BUCKETS; li++)
			{
				for (int r = 0; r < sh; r++)
					for (int c = 0; c < w; c++)
						rd_packed[(size_t)r * w + c] = s_rd_gen_layers[li].pix(r, c);
				retrodepth_write_layer((u32)li, k_names[li], rd_packed.data(), nullptr, (u32)w, (u32)sh);
			}
			retrodepth_commit();
		}
