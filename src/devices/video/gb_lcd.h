...
protected:
	...
	inline void retrodepth_plot_bg_pixel(int layer, int x, int y, uint16_t color);
	inline void retrodepth_plot_sprite_pixel(bool front, int x, int y, uint16_t color);

	void select_sprites();
	void calculate_window_cycles();
	virtual void update_sprites();
	virtual void update_scanline(uint32_t cycles_to_go);
	void retrodepth_prepare_scanline(uint16_t backdrop_pen);
	void retrodepth_export_frame(screen_device &screen);
	...
	enum {
		...
		RD_GB_BACKGROUND,
		RD_GB_WINDOW,
		RD_GB_SPRITES_BEHIND,
		RD_GB_SPRITES_FRONT,
		RD_GB_BUCKETS
	};

	bitmap_ind16 m_rd_layers[RD_GB_BUCKETS];
