...
void ppu2c0x_device::start_nopalram()
{
	...
	for (auto &layer : m_rd_layers)
		layer.allocate(VISIBLE_SCREEN_WIDTH, VISIBLE_SCREEN_HEIGHT);
...
void ppu2c0x_device::draw_tile(u8* line_priority, int color_byte, int color_bits, int address, int start_x, u32 back_pen, u32*& dest)
{
	...
	for (int i = 0; i < 8; i++)
	{
		...
		if ((start_x + i) >= 0 && (start_x + i) < VISIBLE_SCREEN_WIDTH)
		{
			...
			if (pix)
			{
				const u16 palval = m_palette_ram[((4 * color) + pix) & 0x1f];
				m_rd_layers[RD_NES_BACKGROUND].pix(m_scanline, start_x + i) =
					pen_color(apply_grayscale_and_emphasis(palval));
			}
...
void ppu2c0x_device::retrodepth_prepare_scanline(u32 back_pen)
{
	if (m_scanline < 0 || m_scanline >= VISIBLE_SCREEN_HEIGHT)
		return;

	const u32 backdrop = pen_color(apply_grayscale_and_emphasis((u8)back_pen));
	for (int x = 0; x < VISIBLE_SCREEN_WIDTH; x++)
	{
		m_rd_layers[RD_NES_BACKDROP].pix(m_scanline, x) = backdrop;
		m_rd_layers[RD_NES_BACKGROUND].pix(m_scanline, x) = 0;
		m_rd_layers[RD_NES_SPRITES_BEHIND].pix(m_scanline, x) = 0;
		m_rd_layers[RD_NES_SPRITES_FRONT].pix(m_scanline, x) = 0;
	}
}

void ppu2c0x_device::retrodepth_export_frame()
{
	if (!retrodepth_active())
		return;

	static std::vector<u32> rd_packed;
	rd_packed.resize((size_t)VISIBLE_SCREEN_WIDTH * VISIBLE_SCREEN_HEIGHT);
	static const char* k_names[RD_NES_BUCKETS] = {
		"backdrop",
		"background",
		"sprites_behind",
		"sprites_front"
	...
	for (int li = 0; li < RD_NES_BUCKETS; li++)
	{
		for (int y = 0; y < VISIBLE_SCREEN_HEIGHT; y++)
			for (int x = 0; x < VISIBLE_SCREEN_WIDTH; x++)
				rd_packed[(size_t)y * VISIBLE_SCREEN_WIDTH + x] = m_rd_layers[li].pix(y, x);
		retrodepth_write_layer((u32)li, k_names[li], rd_packed.data(), nullptr,
			(u32)VISIBLE_SCREEN_WIDTH, (u32)VISIBLE_SCREEN_HEIGHT);
	}
	retrodepth_commit();
}
...
void ppu2c0x_device::draw_sprite_pixel_low(bitmap_rgb32 &bitmap, int pixel_data, int pixel, int sprite_xpos, int color, int sprite_index, u8 *line_priority)
{
	if (is_spritepixel_opaque(pixel_data, color))
	{
		...
		{
			if ((sprite_xpos + pixel) >= 0 && BIT(~line_priority[sprite_xpos + pixel], 0))
			{
				const u16 palval = m_palette_ram[((4 * color) | pixel_data) & 0x1f];
				m_rd_layers[RD_NES_SPRITES_BEHIND].pix(m_scanline, sprite_xpos + pixel) =
					pen_color(apply_grayscale_and_emphasis(palval));
			}
...
void ppu2c0x_device::draw_sprite_pixel_high(bitmap_rgb32 &bitmap, int pixel_data, int pixel, int sprite_xpos, int color, int sprite_index, u8 *line_priority)
{
	if (is_spritepixel_opaque(pixel_data, color))
	{
		if ((sprite_xpos + pixel) < VISIBLE_SCREEN_WIDTH)
		{
			...
			{
				if ((sprite_xpos + pixel) >= 0)
				{
					const u16 palval = m_palette_ram[((4 * color) | pixel_data) & 0x1f];
					m_rd_layers[RD_NES_SPRITES_FRONT].pix(m_scanline, sprite_xpos + pixel) =
						pen_color(apply_grayscale_and_emphasis(palval));
				}
				/* no, draw */
				draw_sprite_pixel(sprite_xpos, color, pixel, pixel_data, bitmap);
...
void ppu2c0x_device::render_scanline()
{
	...
	retrodepth_prepare_scanline(m_back_color);
...
void ppu2c0x_device::update_visible_disabled_scanline()
{
	...
	retrodepth_prepare_scanline(back_pen);
