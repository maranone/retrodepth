...
namespace {
constexpr int RD_SMS_BUCKETS = 4;
enum : int {
	RD_SMS_BACKDROP = 0,
	RD_SMS_BG_LOW,
	RD_SMS_BG_HIGH,
...
static bitmap_rgb32 s_rd_sms_layers[RD_SMS_BUCKETS];
static u32 s_rd_sms_cap_bg_low[256];
static u32 s_rd_sms_cap_bg_high[256];
static u32 s_rd_sms_cap_spr[256];
static bool s_rd_sms_capture_this_line = false;
}
...
void sega315_5124_device::draw_leftmost_pixels_mode4(int *line_buffer, int *priority_selected, int fine_x_scroll, int palette_selected, int tile_line)
{
	...
	for (int pixel_plot_x = 0; pixel_plot_x < fine_x_scroll; pixel_plot_x++)
	{
		...
		if (s_rd_sms_capture_this_line)
			s_rd_sms_cap_bg_low[pixel_plot_x] = (u32)m_palette_lut->pen(m_current_palette[pen_bit_1 ? 0x10 : 0x00]);
	}
}
	...
	for (int tile_column = 0; tile_column < 32; tile_column++)
	{
		...
		for (int pixel_x = 0; pixel_x < 8; pixel_x++)
		{
			...
			if (pixel_plot_x < 256)
			{
				...
				if (s_rd_sms_capture_this_line)
				{
					const u32 color = (u32)m_palette_lut->pen(m_current_palette[pen_selected]);
					if (priority_select)
						s_rd_sms_cap_bg_high[pixel_plot_x] = color;
					else
						s_rd_sms_cap_bg_low[pixel_plot_x] = color;
				}
			}
		}
	}
		...
		for (int pixel_x = 0; pixel_x < 8; pixel_x++)
		{
			...
			for (int zoom = 0; zoom < zoom_scale; zoom++)
			{
				...
				if (!(priority_selected[pixel_plot_x] & PRIORITY_BIT))
				{
					...
					if (s_rd_sms_capture_this_line)
						s_rd_sms_cap_spr[pixel_plot_x] = (u32)m_palette_lut->pen(m_current_palette[pen_selected]);
				}
				else
				{
...
TIMER_CALLBACK_MEMBER(sega315_5124_device::draw_scanline)
{
	...
	int blitline_buffer[256];
	int priority_selected[256];
	const bool rd_export_line = retrodepth_active() && line >= 0 && line < m_frame_timing[ACTIVE_DISPLAY_V];
	s_rd_sms_capture_this_line = rd_export_line && !m_display_disabled && m_vdp_mode == 4;
	if (rd_export_line)
	{
		if (line == 0)
		{
			for (int li = 0; li < RD_SMS_BUCKETS; li++)
			{
				if (s_rd_sms_layers[li].width() != 256 || s_rd_sms_layers[li].height() != m_frame_timing[ACTIVE_DISPLAY_V])
					s_rd_sms_layers[li].allocate(256, m_frame_timing[ACTIVE_DISPLAY_V]);
				s_rd_sms_layers[li].fill(1u);
			}
		}
		std::fill(std::begin(s_rd_sms_cap_bg_low), std::end(s_rd_sms_cap_bg_low), 1u);
		std::fill(std::begin(s_rd_sms_cap_bg_high), std::end(s_rd_sms_cap_bg_high), 1u);
		std::fill(std::begin(s_rd_sms_cap_spr), std::end(s_rd_sms_cap_spr), 1u);
	}
	...
	{
		const int w = 256;
		const int h = m_frame_timing[ACTIVE_DISPLAY_V];
		const u32 backdrop_pixel = (u32)m_palette_lut->pen(m_current_palette[BACKDROP_COLOR]);
		for (int x = 0; x < w; x++)
		{
			s_rd_sms_layers[RD_SMS_BACKDROP].pix(line, x) = backdrop_pixel;
			s_rd_sms_layers[RD_SMS_BG_LOW].pix(line, x) = s_rd_sms_cap_bg_low[x];
			s_rd_sms_layers[RD_SMS_BG_HIGH].pix(line, x) = s_rd_sms_cap_bg_high[x];
			s_rd_sms_layers[RD_SMS_SPRITES].pix(line, x) = s_rd_sms_cap_spr[x];
		}
		...
		if (line == h - 1)
		{
			...
			static const char* k_names[RD_SMS_BUCKETS] = {
				"backdrop",
				"bg_low",
				"bg_high",
				"sprites"
			...
			for (int li = 0; li < RD_SMS_BUCKETS; li++)
			{
				for (int r = 0; r < h; r++)
					for (int c = 0; c < w; c++)
						rd_packed[(size_t)r * w + c] = s_rd_sms_layers[li].pix(r, c);
	...
	s_rd_sms_capture_this_line = false;
}
