...
void dmg_ppu_device::common_start()
{
	...
	for (int i = 0; i < RD_GB_BUCKETS; i++)
		screen().register_screen_bitmap(m_rd_layers[i]);
...
inline void dmg_ppu_device::retrodepth_plot_bg_pixel(int layer, int x, int y, uint16_t color)
{
	if (x >= 0 && x < m_bitmap.width() && y >= 0 && y < m_bitmap.height())
		m_rd_layers[layer].pix(y, x) = color;
}

inline void dmg_ppu_device::retrodepth_plot_sprite_pixel(bool front, int x, int y, uint16_t color)
{
	int layer = front ? RD_GB_SPRITES_FRONT : RD_GB_SPRITES_BEHIND;
	if (x >= 0 && x < m_bitmap.width() && y >= 0 && y < m_bitmap.height())
		m_rd_layers[layer].pix(y, x) = color;
}

void dmg_ppu_device::retrodepth_prepare_scanline(uint16_t backdrop_pen)
{
	if (m_current_line < 0 || m_current_line >= m_bitmap.height())
		return;

	rectangle r(0, m_bitmap.width() - 1, m_current_line, m_current_line);
	m_rd_layers[RD_GB_BACKDROP].fill(backdrop_pen, r);
	for (int i = 1; i < RD_GB_BUCKETS; i++)
		m_rd_layers[i].fill(0, r);
}

void dmg_ppu_device::retrodepth_export_frame(screen_device &screen)
{
	...
	static const char* k_names[RD_GB_BUCKETS] = {
		"backdrop",
		"background",
		"window",
		"sprites_behind",
	...
	for (int li = 0; li < RD_GB_BUCKETS; li++)
	{
		for (int y = 0; y < h; y++)
		{
			for (int x = 0; x < w; x++)
			{
				u16 pen = m_rd_layers[li].pix(y, x);
				// Non-backdrop buckets are cleared to pen 0 between scanlines.
				// Export those untouched pixels as the RetroDepth transparent sentinel,
				// not as palette entry 0, so deeper layers remain visible through them.
				rd_packed[(size_t)y * w + x] =
					(li != RD_GB_BACKDROP && pen == 0)
						? 1u
						: (u32)screen.palette().pen_color(pen);
			}
		}
		retrodepth_write_layer((u32)li, k_names[li], rd_packed.data(), nullptr, (u32)w, (u32)h);
	}
	retrodepth_commit();
}
...
{
	retrodepth_prepare_scanline((!m_gbc_mode) ? m_gb_bpal[0] : 32767);
		...
		if (m_line.drawing)
		{
			...
			else
			{
				if (!m_line.starting && m_line.tile_cycle < 8)
				{
					...
					uint16_t bg_pen = m_gb_bpal[m_line.shift_register >> 14];
					plot_pixel(m_line.pixels_drawn, m_current_line, bg_pen);
					if ((m_line.window_active || (LCDCONT & BACKGROUND_ENABLED))
						&& ((m_line.shift_register >> 14) != 0))
					{
						retrodepth_plot_bg_pixel(
							m_line.window_active ? RD_GB_WINDOW : RD_GB_BACKGROUND,
							m_line.pixels_drawn, m_current_line, bg_pen);
					}
	...
	for (int i = m_sprCount - 1; i >= 0; i--)
	{
		...
		case 0xA0:                 /* priority is set (behind bgnd & wnd, flip x) */
			for (int bit = 0; bit < 8; bit++, xindex++)
			{
				...
				if (colour && !m_bg_zbuf[xindex] && xindex >= 0 && xindex < 160)
				{
					...
					retrodepth_plot_sprite_pixel(false, xindex, yindex, spal[colour]);
				}
				data >>= 1;
			}
			break;
		case 0x20:                 /* priority is not set (overlaps bgnd & wnd, flip x) */
			for (int bit = 0; bit < 8; bit++, xindex++)
			{
				...
				{
					plot_pixel(xindex, yindex, spal[colour]);
					retrodepth_plot_sprite_pixel(true, xindex, yindex, spal[colour]);
				}
				data >>= 1;
			}
			break;
		...
		if (cycles_to_go < 160)
		{
			...
			while (l < 2)
			{
				...
				while (i > 0)
				{
					...
					{
						int colour = ((data & 0x8000) ? 2 : 0) | ((data & 0x0080) ? 1 : 0);
						uint16_t pen = m_gb_bpal[colour];
						plot_pixel(xindex, m_current_line, pen);
						if (colour != 0)
							retrodepth_plot_bg_pixel(l ? RD_GB_WINDOW : RD_GB_BACKGROUND,
								xindex, m_current_line, pen);
		...
		case 0xA0:                 /* priority is set (behind bgnd & wnd, flip x) */
			for (int bit = 0; bit < 8; bit++, xindex++)
			{
				...
				if (colour && !m_bg_zbuf[xindex] && xindex >= 0 && xindex < 160)
				{
					...
					retrodepth_plot_sprite_pixel(false, xindex, yindex, m_cgb_spal[pal + colour]);
				}
				data >>= 1;
			}
			break;
			...
			for (int bit = 0; bit < 8; bit++, xindex++)
			{
				...
				if (colour && xindex >= 0 && xindex < 160)
				{
					if (!m_gbc_mode)
						colour = pal ? m_gb_spal1[colour] : m_gb_spal0[colour];
					plot_pixel(xindex, yindex, m_cgb_spal[pal + colour]);
					retrodepth_plot_sprite_pixel(true, xindex, yindex, m_cgb_spal[pal + colour]);
				}
				data >>= 1;
			}
			break;
			...
			while (l < 2)
			{
				...
				while (i > 0)
				{
					while ((m_layer[l].xshift < 8) && i)
					{
						...
						uint16_t pen = m_cgb_bpal[(!m_gbc_mode) ? m_gb_bpal[colour] : (((gbcmap[m_layer[l].xindex] & 0x07) * 4) + colour)];
						plot_pixel(xindex, m_current_line, pen);
						if (colour != 0)
							retrodepth_plot_bg_pixel(l ? RD_GB_WINDOW : RD_GB_BACKGROUND,
								xindex, m_current_line, pen);
...
uint32_t dmg_ppu_device::screen_update(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	retrodepth_export_frame(screen);
	return 0;
}
